...
# #region agent log
_DEBUG_LOG_PATH = Path(__file__).resolve().parent.parent.parent.parent / ".cursor" / "debug-8a19c1.log"
        ...
        with open(_DEBUG_LOG_PATH, "a") as f:
            f.write(
                json.dumps(
                    {
                        "sessionId": "8a19c1",
                        "location": "train_stage2_clean.py",
                        "message": msg,
                        "data": data,
...
# Paths: script at experiments/KIT-Experiment/scripts/training/
_KIT_SCRIPT_DIR = Path(__file__).resolve().parent
_KIT_EXPERIMENT_ROOT = _KIT_SCRIPT_DIR.parent.parent
_KIT_GARAGE_ROOT = _KIT_EXPERIMENT_ROOT.parent.parent
sys.path.insert(0, str(_KIT_GARAGE_ROOT))
...
def _binary_metrics_from_predictions(
    ...
    if window_scores is None:
        window_scores = sensor_pred.max(dim=1).values
...
def train_stage2_clean(
    ...
    hard_ratio_start: float = 0.05,
    hard_ratio_end: float = 0.2,
    hard_ratio_switch_epoch: int = 8,
    ...
    best_checkpoint_path = os.path.join(checkpoint_dir, "stage2_clean_best.pt")
    ...
    for epoch in range(num_epochs):
        ...
        with torch.no_grad():
            for vb_idx, (
                ...
                # Inference: ensemble global + sensor_max for robustness
                window_score_inference = (
                    0.7 * torch.sigmoid(global_logits.squeeze(-1))
                    + 0.3 * sensor_probs.max(dim=-1).values
                )
        ...
        _debug_log(
            "epoch_end",
            {
                ...
                "weighted_window": lambda_window * val_loss_window,
            },
            "H1_H3_H4",
        )
        # #endregion
        ...
        if calibrate_every > 0 and (
            ...
            with torch.no_grad():
                for (
                    ...
                    window_score_inference = (
                        0.7 * torch.sigmoid(global_logits.squeeze(-1))
                        + 0.3 * torch.sigmoid(sensor_logits).max(dim=-1).values
                    )
            ...
            cal_metrics = _binary_metrics_from_predictions(
                ...
                sensor_threshold=calibrated_sensor_thr,
            )
        else:
    ...
    with torch.no_grad():
        for loader in (train_loader, val_loader):
            for X_batch, _, sensor_labels_batch, window_labels_batch in loader:
                X_batch = X_batch.to(device)
                sensor_logits, _, _ = model(X_batch, return_global=True, return_sensor_embeddings=True)
                sensor_probs = torch.sigmoid(sensor_logits)
                window_scores = sensor_probs.max(dim=-1).values.cpu().numpy()
    ...
    print(f"[Calibration] Best train+val threshold: {best_thresh:.4f} (F1={best_f1:.4f})")
    checkpoint["calibrated_threshold_trainval"] = {"window": float(best_thresh), "f1": float(best_f1)}
...
def main():
    ...
    parser.add_argument(
        "--data_path",
        type=str,
        default=str(_KIT_EXPERIMENT_ROOT / "data" / "formatted"),
        help="Path to data directory",
    )
    parser.add_argument(
        "--stage1_checkpoint",
        type=str,
        default=str(_KIT_EXPERIMENT_ROOT / "checkpoints" / "stage1_best_forecast.pt"),
    ...
    parser.add_argument(
        "--checkpoint_dir",
        type=str,
        default=str(_KIT_EXPERIMENT_ROOT / "checkpoints"),
    ...
    parser.add_argument(
        "--use_calibrated_threshold_for_checkpoint",
        action="store_true",
        help="Persist latest calibrated thresholds in checkpoint",
    )
    parser.add_argument(
        "--use_focal_loss",
        action="store_true",
    ...
    if os.path.exists(train_npz) and os.path.exists(val_npz):
        ...
        X_train_sensor = torch.tensor(train_data["X"], dtype=torch.float32)
        train_sensor_labels = torch.tensor(train_data["y_sensor"], dtype=torch.float32)
        train_window_labels = torch.tensor(train_data["y_window"], dtype=torch.long)
        X_val_sensor = torch.tensor(val_data["X"], dtype=torch.float32)
        val_sensor_labels = torch.tensor(val_data["y_sensor"], dtype=torch.float32)
        val_window_labels = torch.tensor(val_data["y_window"], dtype=torch.long)
        ...
        print(f"\nLoading data from CSV: {args.data_path}")
        df_list = []
        ...
        unique_drives = np.sort(data[ID_COL].unique())  # sort first for determinism
        rng = np.random.default_rng(args.seed)
        shuffled_drives = rng.permutation(unique_drives)
        n_drives = len(shuffled_drives)
        train_drives = shuffled_drives[: int(0.70 * n_drives)]
        all_val_drives = shuffled_drives[int(0.70 * n_drives) : int(0.85 * n_drives)]
        val_drives   = all_val_drives   # all val drives used for checkpoint selection
        calib_drives = all_val_drives   # same drives used for threshold calibration
        test_drives = shuffled_drives[int(0.85 * n_drives) :]

        print(f"Drive split — Train: {len(train_drives)}, Val/Calib: {len(val_drives)}, Test: {len(test_drives)}")
        for split_name, split_drives in [
            ("Train", train_drives),
            ("Val", val_drives),
            ("Calib", calib_drives),
            ("Test", test_drives),
            ...
            type_counts = {}
            for d in split_drives:
                prefix = "".join(filter(str.isalpha, str(d)))
                type_counts[prefix] = type_counts.get(prefix, 0) + 1
            print(f"  {split_name} drives ({len(split_drives)}): {type_counts}")

        train_data = data[data[ID_COL].isin(train_drives)].copy()
        val_data = data[data[ID_COL].isin(val_drives)].copy()
        calib_data = data[data[ID_COL].isin(calib_drives)].copy()
        ...
        X_calib, y_calib, _ = build_clean_windows(
            calib_data, SENSOR_COLS, ID_COL, TIME_COL, WINDOW_SIZE, scaler=scaler_train
        )
        ...
        print(
            f"\nInjecting faults with sensor-level labels (stratified, fault_rate={fault_rate:.0%})..."
        )
        ...
        X_calib_sensor, _, calib_sensor_labels, calib_window_labels, _ = (
            inject_faults_with_sensor_labels(
                X_calib,
                y_calib,
                SENSOR_COLS,
    ...
    train_stage2_clean(
        ...
        use_calibrated_threshold_for_checkpoint=args.use_calibrated_threshold_for_checkpoint,
        num_workers=args.num_workers,
