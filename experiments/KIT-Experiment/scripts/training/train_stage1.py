...
# Paths: script lives at experiments/KIT-Experiment/scripts/training/
_SCRIPT_DIR = Path(__file__).resolve().parent
_EXPERIMENT_ROOT = _SCRIPT_DIR.parent.parent
_GARAGE_ROOT = _EXPERIMENT_ROOT.parent.parent
sys.path.insert(0, str(_GARAGE_ROOT))
from models.gdn_model import GDN

torch.set_default_dtype(torch.float32)

# ============================================================================
# Constants (KIT experiment defaults)
# ============================================================================

DATA_PATH = str(_EXPERIMENT_ROOT / "data" / "dataset")
SENSOR_COLS = [
    "ENGINE_RPM ()",
    "VEHICLE_SPEED ()",
    "THROTTLE ()",
...
def train_stage1(
    ...
    patience_counter = 0
    max_patience = (
        early_stopping_patience if not focus_on_contrastive else 999999
    )
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        if optimizer.param_groups[0]["lr"] < lr_before:
            print(f"  ↓ LR reduced: {lr_before:.2e} → {optimizer.param_groups[0]['lr']:.2e}")
...
def main():
    ...
    parser.add_argument(
        "--checkpoint_dir",
        type=str,
        default=str(_EXPERIMENT_ROOT / "checkpoints"),
    ...
    # Load data: from .npz if available, else from CSV
    ...
    if os.path.exists(train_npz) and os.path.exists(val_npz):
        ...
        X_key = "X_clean" if "X_clean" in train_data else "X"
        X_train = torch.tensor(train_data[X_key], dtype=torch.float32)
        y_train_forecast = torch.tensor(train_data["y_forecast"], dtype=torch.float32)
        drive_ids_train = train_data["drive_ids"]
        X_val = torch.tensor(val_data[X_key], dtype=torch.float32)
        y_val_forecast = torch.tensor(val_data["y_forecast"], dtype=torch.float32)
        ...
        data = downsample(data, time_col=TIME_COL, source_file_col=ID_COL, downsample_factor=2)
        ...
        train_data_df = data[data[ID_COL].isin(train_drives)].copy()
        val_data_df = data[data[ID_COL].isin(val_drives)].copy()
        X_train, y_train_forecast, drive_ids_train, scaler_train = build_forecast_windows(
            train_data_df, SENSOR_COLS, ID_COL, TIME_COL, WINDOW_SIZE,
            horizons=FORECAST_HORIZONS, scaler=None,
        )
        X_val, y_val_forecast, drive_ids_val, _ = build_forecast_windows(
            val_data_df, SENSOR_COLS, ID_COL, TIME_COL, WINDOW_SIZE,
            horizons=FORECAST_HORIZONS, scaler=scaler_train,
        )
        print(f"Train windows: {len(X_train)}, Val windows: {len(X_val)}")
